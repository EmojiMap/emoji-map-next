...
describe('useClerkUsers', () => {
  ...
  it('successfully fetches users with default pagination', async () => {
    ...
    const { result } = renderHook(() => useClerkUsers(), { wrapper });
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      '/api/admin/clerk-users?limit=10&offset=0'
    );
  });
  ...
  it('fetches users with custom pagination parameters', async () => {
    ...
    const { result } = renderHook(
      () => useClerkUsers({ limit: 20, offset: 40 }),
      { wrapper }
    );
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      '/api/admin/clerk-users?limit=20&offset=40'
    );
  });
